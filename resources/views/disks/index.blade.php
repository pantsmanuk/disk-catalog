...
@section('content')
    ...
    <section class="hero">
        ...
        <dl class="metrics">
            <div><dt>Occupied</dt><dd>{{ $occupied }}<small>/24</small></dd></div>
    ...
    <section class="panel">
        <header class="panel-heading">
            <div><p class="eyebrow">Front enclosure</p><h2>Bays 01–24</h2></div>
        ...
        <div class="shelf" aria-label="Front disk bays">
            @foreach (range(1, 24) as $slot)
                ...
                @if ($disk)
                    <a class="bay {{ strtolower($disk->interface) }} status-{{ $disk->status->value }}" href="{{ route('disks.show', $disk) }}" aria-label="Bay {{ $slot }}, {{ $disk->status->label() }}, {{ $disk->serial }}">
                        <span class="bay-slot">{{ str_pad($slot, 2, '0', STR_PAD_LEFT) }}</span>
